#include "forms/deduplicateListSubWindow.hpp"
#include "ui_deduplicateListSubWindow.h"
...
void deduplicateListSubWindow::on_deleteButton_clicked() {
    ...
    if (items.empty()) {
        auto w_goodNews = new goodNewsWindow(this, "请先在右侧列表中选择你要删除的图书行");
        w_goodNews->show();
        return;
    }
...
void deduplicateListSubWindow::on_backSaveButton_clicked() {
    auto w_editBook = new editBookWindow(nullptr, newList);
    w_editBook->show();
    this->close();
}
