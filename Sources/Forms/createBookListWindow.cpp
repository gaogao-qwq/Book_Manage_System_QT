#include "forms/createBookListWindow.hpp"
#include "ui_createBookListWindow.h"


createBookListWindow::createBookListWindow(QWidget *parent, BookList *list) :
        QWidget(parent), ui(new Ui::createBookListWindow), bookList(list) {
    ui->setupUi(this);
    // 不允许编辑
...
void createBookListWindow::on_backButton_clicked() {
    auto w_mainMenu = new main_menu_window(nullptr, bookList);
    w_mainMenu->show();
    this->close();
}

void createBookListWindow::on_backSaveButton_clicked() {
    // 若原图书表存在（长度大于 0）弹出警告
    if (bookList->size > 0) {
...
void createBookListWindow::on_inputButton_clicked() {
    if (newList->size != 0) {
...
createBookListWindow::~createBookListWindow() {
    delete ui;
}
